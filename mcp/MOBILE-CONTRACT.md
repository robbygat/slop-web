# Slop MCP bridge v1 — implemented, deployment pending

Base: `https://api.slop.game/functions/v1/slop-mcp`. All requests return JSON and
`Cache-Control: no-store`. Phone routes require `Authorization: Bearer <current
Supabase access JWT>`. Never send refresh tokens, user IDs as authorization, or
the desktop connection token. No browser Origin is accepted by this first release.
...
confirmation_uri}`. The local adapter generates/stores its own secret first.
