...
export const searchPeople=(term='')=>result(supabase.from('profiles').select('id,username,display_name,avatar_url,bio,slop_look').ilike('username',`%${term.replace(/[%_\\]/g,'').slice(0,40)}%`).order('username').limit(24));
