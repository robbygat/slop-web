...
export async function encodeCapture(frames,target=null){
 ...
 for(let index=0;index<frames.length;index++){
  ...
  try{
   if(index===0){({width,height}=captureDimensions(bitmap.width,bitmap.height,target));coverCanvas=canvasFor(bitmap,width*2,height*2);}
   const canvas=canvasFor(bitmap,width,height),rgba=canvas.getContext('2d',{willReadFrequently:true}).getImageData(0,0,width,height).data;
