...
export default function Download(){
 ...
 return <div className="download-page"><header className="download-heading"><h1>Slop, on your phone.</h1><p>Play, make games and keep your Slop with you.<br/>One account on the web, iPhone and Android.</p></header>
  ...
  <section className="download-platform"><h2>Android</h2><p>Download the APK directly to your Android phone.</p>{release.data?<><a className="store-link" href={release.data.url} download={release.data.file}><Icon name="download"/><span>Download Slop for Android</span><Icon name="arrow" size={18}/></a><p className="release-summary">APK · Version {release.data.version} · {release.data.size}</p><details className="apk-details"><summary>Installation and release details</summary><p>Open the downloaded APK. If Android asks, allow installation from your browser, then choose Install.</p><p>Built {release.data.date}<br/>{release.data.architectures||'Android ARM64'} · Android {release.data.minAndroid||'7.0'} or later</p><details><summary>Verify your download</summary><p>SHA-256</p><code className="checksum">{release.data.sha256}</code><p>Mobile source: <code>{release.data.commit}</code></p></details></details></>:<p className="apk-pending" role="status">{release.loading?'Checking the latest release…':'The next APK is being prepared.'}</p>}</section></div>
