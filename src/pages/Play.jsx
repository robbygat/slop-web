...
export default function Play({params,search,term,setSearch}){
 ...
 {!search&&(popular.loading||popular.error||popular.data?.length>0)&&<section className="popular-section"><SectionHeading title="Popular games" action={<a className="inline-link" href="#/feed">Keep playing <Icon name="arrow" size={18}/></a>}></SectionHeading>{popular.loading?<div className="game-grid skeleton-grid" role="status" aria-label="Loading popular games">{[0,1,2,3].map(i=><div className="skeleton-card" key={i}/>)}</div>:popular.error?<Notice error={popular.error} onRetry={popular.refresh}/>:<div className="game-grid">{popular.data?.map(game=><GameCard key={game.id} game={game} onOpen={setSelected}/>)}</div>}</section>}
