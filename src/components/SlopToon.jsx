...
export function SlopToon({look,body='ghost',color='tangerine',className='',alt='Your Slop',controls=true,autoRotate=false,...props}){
 const canvas=useRef(null),stage=useRef(null),pose=useRef({angle:0,velocity:0,drag:null,reactionAt:-Infinity,lastInput:-Infinity});
 ...
 useEffect(()=>{
  let disposed=false,raf=0,visible=true,lastFrame=0,renderer,snapshot;const started=performance.now();const node=canvas.current,host=stage.current,ctx=node.getContext('2d');
  if(!ctx)return;
  ...
  function draw(now){
   ...
   if(!p.drag&&!reduced.matches){p.angle+=p.velocity*elapsed;p.velocity*=Math.exp(-elapsed*3.25);}
   const sinceTouch=(now-p.lastInput)/1000;
   const idle=reduced.matches||p.drag||sinceTouch<3?0:Math.sin(now/5000)*.15;
   ...
   const angle=p.angle+idle+spin;p.drawnAngle=angle;const reaction=reduced.matches?0:Math.max(0,Math.min(1,(now-p.reactionAt)/900));
   renderer.drawSlopToon(ctx,snapshot,{width:node.width,height:node.height,time:reduced.matches?0:now/1000,angle,reducedMotion:reduced.matches,reaction});
  ...
  function motion(){pose.current.velocity=0;cancelAnimationFrame(raf);raf=0;start();}
  control.current={draw:start,reset:()=>{pose.current.angle=0;pose.current.velocity=0;pose.current.lastInput=performance.now();setTurn(0);start();}};
 ...
 function begin(e){if(e.button!==0&&e.pointerType==='mouse')return;const p=pose.current;p.angle=p.drawnAngle??p.angle;p.drag={id:e.pointerId,startX:e.clientX,x:e.clientX,angle:p.angle,time:performance.now()};p.velocity=0;p.lastInput=performance.now();e.currentTarget.setPointerCapture(e.pointerId);control.current?.draw();}
 function move(e){const p=pose.current,d=p.drag;if(!d||d.id!==e.pointerId)return;const now=performance.now(),width=Math.max(100,stage.current.clientWidth),delta=e.clientX-d.x;p.angle=d.angle+(e.clientX-d.startX)/width*tau;p.velocity=Math.max(-5,Math.min(5,delta/width*tau/Math.max(.016,(now-d.time)/1000)));d.x=e.clientX;d.time=now;p.lastInput=now;control.current?.draw();}
 function end(e){const p=pose.current,d=p.drag;if(!d||d.id!==e.pointerId)return;if(Math.abs(e.clientX-d.startX)<6)p.reactionAt=performance.now();p.drag=null;p.lastInput=performance.now();setTurn(Math.round(normalize(p.angle)*180/Math.PI));control.current?.draw();}
 function keyboard(e){if(!['ArrowLeft','ArrowRight','Home','End'].includes(e.key))return;e.preventDefault();pose.current.angle=e.key==='Home'?0:e.key==='End'?Math.PI:pose.current.angle+(e.key==='ArrowLeft'?-1:1)*Math.PI/12;pose.current.velocity=0;pose.current.lastInput=performance.now();setTurn(Math.round(normalize(pose.current.angle)*180/Math.PI));control.current?.draw();}
