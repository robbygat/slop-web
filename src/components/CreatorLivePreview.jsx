...
import {gameTargetFromFiles,previewGameForTarget} from '../lib/game-target-contract.js';
export default function CreatorLivePreview({project,revision,paused=false,working=false}){
 ...
 const target=gameTargetFromFiles(revision.files),game=previewGameForTarget(target);
 return <div className="creator-live-preview"><div className="creator-preview-label"><span>{working?'Saved version · your update is being built':'Your playable game'}</span><span>{target==='cross-platform'?'Phone + desktop':target==='desktop'?'Desktop':'Phone'} · Private</span></div>{prepared?.previewKey===identity?<GamePlayer key={identity} url={prepared.url} game={game} preview paused={paused} initialMuted title={revision.title||project.title}/>:error?<Notice error={error} onRetry={()=>setRetry(v=>v+1)}/>:<div className="creator-preview-loading" role="status"><span className="loader"/>Opening this version…</div>}{prepared?.previewKey===identity&&error&&<Notice error={error}/>}</div>;
}
