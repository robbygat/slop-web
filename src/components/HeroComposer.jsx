...
export default function HeroComposer({className='',placeholder='Type or speak your game idea'}){
 ...
 return <div className={`hero-composer ${listening?'is-listening':''} ${className}`}>
  ...
  {(error||listening)&&<p id={label+'-status'} className="hero-composer-status" role={error?'alert':'status'}>{error||'Listening… describe the game you want.'}</p>}
 </div>;
}
