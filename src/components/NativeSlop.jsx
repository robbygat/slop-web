...
export function NativeSlop({look,body='ghost',color='tangerine',className='',alt='Your Slop',controls=true,autoRotate=false,paused=false}){
 ...
 function send(front=false){if(!state.current.ready)return;frame.current?.contentWindow?.postMessage(nativeCharacterMessage(JSON.parse(currentLook.current),{reducedMotion:state.current.reducedMotion,paused:state.current.paused||!state.current.visible||document.hidden,front,autoRotate:state.current.autoRotate,requestId:++state.current.requestId}),'*');}
 useEffect(()=>{const reduced=matchMedia('(prefers-reduced-motion: reduce)');state.current.reducedMotion=reduced.matches;
  function receive(event){if(nativeCharacterRendered(event,frame.current?.contentWindow,state.current.requestId)){setReady(true);return;}const performance=nativeCharacterPerformance(event,frame.current?.contentWindow);if(performance){host.current.dataset.paintFps=performance.fps.toFixed(1);host.current.dataset.sampleFrames=String(performance.frames);return;}if(!nativeCharacterReady(event,frame.current?.contentWindow))return;state.current.ready=true;send();}
